...
h1<-function(y,delta.t.s,mu,alpha,lambda,rho,sigma){
  #y:degradation measure
  #t:measure interval
  #mu:mean
  #lambda: dispersion
  #rho:model coefficient rho=0 for wiener process
  for(j in 1:2){
         y[,(2+j)]<-pdf(y[,(2+j)],delta.t.s[,j],alpha[j]*mu,lambda[j],rho[j])
    }
...
##################################likelihood for MLE
para_M_step<-function(alpha,lambda,rho,Gam,y,t,par){
  n<-ncol(par)
  ...
  for(i in 1:n){
   x<-0
    for(k in 1:m){
      x<-d(y[i,k],tau[k],alpha*par[,i],rho)
...
lambda.F<-function(alpha,rho,Gam,y,t,par){
  n<-ncol(par)
  tau<-(apply(lambda.t(t,Gam),2,diff))
  m<-length(tau)
  y<-matrix(y,nrow=n,ncol=m,byrow=T)
...
##################################AIC value 
aic<-function(alpha.s,lambda.s,rho.s,Gam.s,sigma.s,Y,delta.t.s,par,n,m){
  ...
  for(i in 1:n){
    ...
    for(j in 1:2){
      Q<-0
      for(k in 1:m){
        Q<-d(x[k,j],delta.t.s[k,j],alpha.s[j]*par[,i],rho.s[j])
  ...
  return(sum(Y[,3:4])+sum(P))
}
##########################################

Reliability<-function(alpha.s,lambda.s,rho.s,Gam.s,sigma.s,t){
  Df<-c(10,15)
  if(rho.s[1]==2){
    par<-1/rgamma(100000,shape=sigma.s,scale=1/sigma.s)
  }else if(rho.s[1]==3){
  ...
  x1<-sqrt(lambda.s[1]/(par^(rho.s[1])*alpha.s[1]^rho.s[1]))*(par*alpha.s[1]*sqrt(delta.t.s[1])-Df[1]/(sqrt(delta.t.s[1])))
  x2<-sqrt(lambda.s[2]/(par^(rho.s[2])*alpha.s[2]^rho.s[2]))*(par*alpha.s[2]*sqrt(delta.t.s[2])-Df[2]/(sqrt(delta.t.s[2])))
      F1<-mean(pnorm(x1))
      F2<-mean(pnorm(x2))
