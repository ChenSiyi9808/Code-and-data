#load(file='F:\\degradation analysis\\Bivariate exponential model\\Code\\SimulationW.Rdata')
load(file='C:\\Users\\syephd\\Desktop\\Bivariate exponential model\\Bivariate exponential model\\Code\\SimulationW.Rdata')
#Data<-read.csv('F:\\degradation analysis\\Bivariate exponential model\\Code\\cracktrain.csv')
...
while(Delta>1e-5){
  ...
  for(i in 1:n){
    ...
    y<-x[nrow(x),3:4]
    Sigma.s<-(sum(lambda.s* alpha.s^2*apply(delta.t.s,2,cumsum)[nrow(delta.t.s),])+sigma.s^(-2))
    Emu[i]<-(sum(lambda.s* alpha.s*y)+sigma.s^(-2))/Sigma.s
  ...
  for(i in 1:n){
    ...
    for(k in 1:m){
      z[k,]<-(x[k,]^2-2*x[k,]*delta.t.s[k,]*alpha.s*Emu[i]+alpha.s^2*delta.t.s[k,]^2*Emu2[i])/delta.t.s[k,]
    }
    Z[Y$unit==i,]<-z
  }
  ...
  Gam.f<-function(j){
    fit<-function(Gam){
      ...
      for(i in 1:n){
        ...
        s[i]<-sum(-log(tau)-(x[,j+2]^2-2*alpha.s[j]*x[,j+2]*tau*Emu[i]+alpha.s[j]^2*tau^2*Emu2[i])/(tau/lambda.s[j]))
      }
      s<--sum(s)
      return(s)
    }
  ...
  library(doParallel)
  Gam.s<-unlist(foreach(i=1:length(alpha.s))%do% Gam.f(i))
...
for(i in 1:n){
  ...
  for(j in 1:2){
    for(k in 1:m){
      x[k,j]<-0.5*log(lambda.s[j]/(2*pi*delta.t.s[k,j]))-(x[k,j]^2-2*x[k,j]*alpha.s[j]*delta.t.s[k,j]*Emu[i]+alpha.s[j]^2*delta.t.s[k,j]^2*Emu2[i])/((2*delta.t.s[k,j])/lambda.s[j])
    }
  }
